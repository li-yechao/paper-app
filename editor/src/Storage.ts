...
export default class Storage {
  private static TOKEN_KEY = 'PAPER_EDITOR_ACCESS_TOKEN'
  private static _token: Token | null = null
  static get token(): Token | null {
    if (!this._token) {
      this._token = this.paperToken
  ...
  private static PAPER_TOKEN_KEY = 'flutter.PAPER_ACCESS_TOKEN'
  private static get paperToken(): Token | null {
    try {
      const v = JSON.parse(JSON.parse(localStorage.getItem(this.PAPER_TOKEN_KEY) || '"{}"'))
      if (typeof v.accessToken === 'string' && typeof v.refreshToken === 'string') {
