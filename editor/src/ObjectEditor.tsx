...
export default forwardRef(function ObjectEditor(
  { userId, objectId, onStateChange }: ObjectEditorProps,
  ref: ForwardedRef<ObjectEditorRef>
) {
  useEffect(() => {
  ...
  if (object.error) {
    ...
    return (
      <_ObjectEditor ref={ref} object={object.data.user.object} onStateChange={onStateChange} />
    )
...
const _ObjectEditor = forwardRef(
  (
    {
      object,
      onStateChange,
    ...
    useImperativeHandle(
      ref,
      () => ({
        save: async () => (stateRef.current ? save(stateRef.current) : undefined),
      }),
      []
    )
